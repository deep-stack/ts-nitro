...
export class Objective implements ObjectiveInterface {
  ...
  static async newObjective(
    ...
    try {
      ...
      throw new WrappedError(`could not find channel ${request.channelId}; ${err}`, err as Error);
    }
  ...
  async crank(signer: NitroSigner): Promise<[Objective, SideEffects, WaitingFor]> {
    ...
    try {
      ...
      throw new WrappedError(`error finding a supported state: ${err}`, err as Error);
    }
